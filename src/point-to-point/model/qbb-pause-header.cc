...
PauseHeader::PauseHeader () : m_time(0) {}
 ...
 */
void 
PauseHeader::SetTime (const uint32_t time) 
{
    m_time = time;
}
void 
PauseHeader::SetQLen (const uint32_t qlen) 
{
    m_qlen = qlen;
}
void 
PauseHeader::SetQIndex (const uint8_t qindex) 
{
    m_qindex = qindex;
}
...
{
    return 9; // 4(time)+4(qlen)+1(qindex) Byte
}
void 
...
{
    os  << "Pause frame, pause time=" << m_time 
        << "us, qlen=" << m_qlen << ", qindex=" << m_qindex << std::endl;
}
